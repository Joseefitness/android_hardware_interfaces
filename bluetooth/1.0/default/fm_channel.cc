...
void SendMessage(int fd, const uint8_t* data, size_t length) {
  ...
  if (sent < 0) {
    ALOGE("%s: dropping a %zu byte message: %s", __func__, length,
          strerror(errno));
  }
}
    ...
    switch (message[0]) {
      ...
      case kOpCommand: {
        ...
        if (vendor_interface != nullptr) {
          vendor_interface->Send(HCI_PACKET_TYPE_COMMAND, message + 1,
                                 length - 1);
        }
        break;
      }
...
void ServerLoop(int server_fd) {
  for (;;) {
    int fd = TEMP_FAILURE_RETRY(accept4(server_fd, nullptr, nullptr,
                                        SOCK_CLOEXEC));
    if (fd < 0) {
...
void FmChannel::Start() {
  ...
  std::call_once(once, []() {
    ...
    if (server_fd < 0) {
      ALOGI("No %s socket, the FM radio cannot share the transport",
            kSocketName);
      return;
    }
...
bool FmChannel::DeliverEvent(const hidl_vec<uint8_t>& event) {
  ...
  } else if (size >= 3 && packet[0] == kVendorSpecificEvent &&
             packet[2] == kFmSubEvent) {
    is_fm = true;
  }
